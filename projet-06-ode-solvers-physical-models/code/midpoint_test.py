...
from partie1 import step_midpoint, meth_epsilon, meth_n_step, plot_tangent_field
import pytest


def test_step_midpoint_constant():
    """dy/dt = 1, y(0) = 0 ⇒ y(t) = t"""
    f = lambda t, y: 1
    sol = meth_n_step(y0=0, t0=0, N=10, h=0.1, f=f, step_method=step_midpoint)
    # sol[i] ≈ i*0.1
...
def test_meth_epsilon_accuracy():
    ...
    sol = meth_epsilon(y0=1, t0=0, tf=1.0, eps=1e-5, f=f, step_method=step_midpoint)
...
if __name__ == "__main__":
    ...
    eps = 1e-4
    sol = meth_epsilon(
        y0, t0, tf, eps, f=lambda t, y: y / (1 + t * t), step_method=step_midpoint
    )
    N = len(sol) - 1
    ...
    plt.plot(tvals, sol, label="Point milieu raffinée")
    ...
    ts, ys = meth_n_step(y0, t0, N, h, rot_dyn, step_midpoint)
    ...
    plt.plot(ts, ys[:, 0], label="y1 numérique (Point milieu)")
    plt.plot(ts, ys[:, 1], label="y2 numérique (Point milieu)")
